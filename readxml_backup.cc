...
void readxml()
{
  ...
  double vffls3d[100],vcosalpha[100],vfprob[100];
  ...
  while(eff)
    {
      ...
      TMVA::gTools().ReadAttr(cut, "cutMin_0", vffls3d[i]);
      TMVA::gTools().ReadAttr(cut, "cutMin_1", vcosalpha[i]);
      TMVA::gTools().ReadAttr(cut, "cutMin_2", vfprob[i]);
  ...
  cout<<"dcandffls3d: "<<vffls3d[maxindex]<<endl;
  cout<<"cosalpha: "<<vcosalpha[maxindex]<<endl;
  cout<<"fprob: "<<vfprob[maxindex]<<endl;
  cout<<"################################################"<<endl;
  cout<<endl;
  ...
  leg->Draw();
  //csig->SaveAs(Form("plots/plot_%s/steps/sig-eff_%.1f_%.1f_%.0f.pdf",channel.Data(),ptmin,ptmax,pop));
  csig->SaveAs(Form("plots/plot_%s/SA/sig-eff_%.1f_%.1f.pdf",channel.Data(),ptmin,ptmax));
