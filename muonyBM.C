...
void checkdimuon(TTree* nt, TTree* ntMC, double ptmin, double ptmax, TString cutB,TString cutBMC)
{
   static int count=0;
   count++;
   float BIN_MIN=2.8,BIN_MAX=3.3;
   TH1D* hmumuB = new TH1D("hmumuB","",50,BIN_MIN,BIN_MAX);
   TH1D* hmumuS = new TH1D("hmumuS","",50,BIN_MIN,BIN_MAX);

   nt->Project("hmumuB","mumumass",Form("%s&&((Run>=210498&&Run<=211256&&(y+0.465)>%f&&(y+0.465)<%f)||(Run>=211313&&Run<=211631&&(y-0.465)>%f&&(y-0.465)<%f))",cutB.Data(),ptmin,ptmax,ptmin,ptmax));
   ntMC->Project("hmumuS","mumumass",Form("%s&&y>%f&&y<%f",cutBMC.Data(),ptmin,ptmax));

   hmumuB->Sumw2();
   hmumuB->Scale(1./hmumuB->Integral());
   hmumuS->Scale(1./hmumuS->Integral());

   float meanB = hmumuB->GetMean();
   float meanS = hmumuS->GetMean();

   float maxy;
   TCanvas* cB = new TCanvas(Form("cB%d",count),"",200,10,600,600);
   if(hmumuB->GetMaximum()>hmumuS->GetMaximum()) maxy=hmumuB->GetMaximum();
   else maxy = hmumuS->GetMaximum();

   hmumuB->GetXaxis()->SetTitle("#mu#mu mass/ GeV/c^{2}");
   hmumuB->GetYaxis()->SetTitle("Probability");
   ...
   hmumuS->UseCurrentStyle();
   TLegend* leg1 = new TLegend(0.20,0.75,0.60,0.92);
   leg1->AddEntry(hmumuB,"data","lp");
   leg1->AddEntry(hmumuS,"MC","f");
   leg1->SetFillStyle(kWhite);
   leg1->SetLineColor(kWhite);
   leg1->Draw();

   TLegend* leg1p = new TLegend(0.12,0.55,0.50,0.72);
   ...
   leg1p->Draw();
   cB->SaveAs(Form("plot_%s/dimuon_yDM_%d.png",selection.Data(),count)); 
...
void muonyBM()
{
   ...
   TTree *treeBBp = (TTree*)inputB->Get("ntKp"); 
   TTree *treeBB0 = (TTree*)inputB->Get("ntKstar"); 
   TTree *treeBBs = (TTree*)inputB->Get("ntphi"); 

   TTree *treeSBp = (TTree*)inputSbplus->Get("ntKp"); 
   TTree *treeSB0 = (TTree*)inputSbzero->Get("ntKstar"); 
   ...
   for(int i=0;i<nBins;i++)
     {
       ...
       if(selection=="Bzero_chi2_trkPt") checkdimuon(treeBB0,treeSB0,ptBins[i],ptBins[i+1],cutB0,cutB0MC);
       if(selection=="Bs_chi2_trkPt") checkdimuon(treeBBs,treeSBs,ptBins[i],ptBins[i+1],cutBs,cutBsMC);
     }
}
