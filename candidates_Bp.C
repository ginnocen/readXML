...
TString selection="Bplus_trkPt_incl";
...
void candidates_Bp()
{
   ...
   //TFile* inputS = new TFile("/export/d00/scratch/jwang/nt_BoostedMC_20140427_Kp_TriggerMatchingMuon_EvtBase_skim.root");
   ...
   for(i=0;i<nentriesS;i++)
       ...
       for(int j=0;j<size;j++)
	 {
	   ...
	   if(selection=="Bzero_trkPt")
	     {
	       if((HLT_PAMu3_v1)&&abs(mumumass[j]-3.096916)<0.15&&abs(y[j]+0.465)<1.93&&(pt[j]>10.)&&trk1Pt[j]>0.7&&trk2Pt[j]>0.7&&chi2cl[j]>1.65e-01&&(d0[j]/d0Err[j])>4.16&&cos(dtheta[j])>7.50e-01&&abs(tktkmass[j]-0.89594)<2.33e-01&&abs(mass[j]-5.279)<0.08) count++;
	     }
	   if(selection=="Bs_trkPt") 
	     {
